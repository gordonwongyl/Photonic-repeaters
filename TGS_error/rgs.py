...
class RGS:
    def __init__(self, branch_param: NDArray, N: int, miu: tgs.Miu, 
                 detector: tgs.Detector = tgs.Detector()) -> None:
        ...
        self.logical_qubit = self.get_logical_qubit(branch_param, miu)
        self.prob_X_measurement = self.logical_qubit.R[0]
        self.prob_Z_measurement = (1 - self.logical_qubit.miu_list[0] + self.logical_qubit.miu_list[0] *self.logical_qubit.R[1])**self.logical_qubit.b[0]

        # P_succ between neighbouring nodes
        self.P_succ = (1 - (1-self.prob_BSM)**(N/2)) * self.prob_X_measurement**2 * self.prob_Z_measurement**(N-2)

    def get_logical_qubit(self):
        pass
    ...
    @property
    def miu_leafs(self) -> float:
        pass

    @property
    def prob_BSM(self) -> float:
        eta_d = self.detector.eta_d
        eta_t = (1-self.miu_leafs)**2
        p_dc = self.detector.p_dc
        return 0.5*eta_d**2 * eta_t + p_dc*(-4*eta_d*eta_t + 2*eta_t + 2)*eta_d

class RGS_ancilla(RGS):
    def __init__(self, branch_param: NDArray, N: int, miu: tgs.Miu,
                 detector = tgs.Detector()) -> None:
        super().__init__(branch_param, N, miu, detector)

    def get_logical_qubit(self, branch_param, miu):
        return tgs.Logical_qubit_ancilla(branch_param, miu, self.detector)

    @property
    def miu_leafs(self) -> float:
        return  self.logical_qubit.miu.total_tree_a(delay_apply=False)
...
class RGS_feedback(RGS):
    ...
    def get_logical_qubit(self, branch_param, miu):
        return tgs.Logical_qubit_feedback(branch_param, miu, self.detector) 
    
    @property
    def miu_leafs(self) -> float:
        return  self.logical_qubit.miu.total_tree_f(delay_apply=False, no_feedback=0)
...
class RGS_Error(tgs.Error):
    def __init__(self, rgs:RGS, time: tgs.Time, t_spin_coherence, epsilon_depolarization=0.00005, epsilon_sp_measure_set=None):
        ...
        # No need to consider detector error? The effects of realistic detection is captured in wrong BSM
        self.error_leafs = 2/3*(epsilon_decoh + epsilon_depolarization - 4/3*epsilon_decoh*epsilon_depolarization)
       
        self.BSM_error = rgs.detector.P_tomix_from_BSM_error(eta_t=(1-rgs.miu_leafs)**2)

        # Errors for the logical qubits
        ...
        for lk in range(b0+1): # lk: # of qubits in k+2 level that can only be directly measured
            ...
            p  = R1/(1-miu+miu*R1)
            parity_error_prob = 0.5 * (1-(1-2*self.epsilon_sp_measure[0])**(lk)*(1-2*self.e_I_k[1])**(b0-lk)) # A14
            self.epsilon_Z += counting * (1-p)**lk * p**(b0-lk) * parity_error_prob


def RGS_fidelity(e_m, e_X, e_Z: float, num_core_nodes:int, num_repeater_nodes:int, p_dc:float = 0) -> float:
    m = num_core_nodes/2
    n = num_repeater_nodes
    E_Z = 0.25 - (0.25*(1-2*e_m)**(2*(n+1)) * (1-2*e_X)**(2*n) * (1-p_dc)**(n+1))
    E_X = E_Z

    E_Y = 0.25 + ( 0.25 * (1-2*e_m)**(2*(n+1)) * (1-2*e_X)**(2*n)*(1-p_dc)**(n+1)) - \
        ( 0.5 * (1-2*e_m)**(2*(n+1)) * (1-2*e_X)**n * (1-2*e_Z)**((2*m-2)*n) * (1-p_dc)**(n+1))
...
def effective_key_rate(rgs: RGS, time: tgs.Time, error:RGS_Error, M:int, n:int, L:float = 1000e3, L_ATT: float =20e3):
    fidelity = RGS_fidelity(error.error_leafs, error.epsilon_X, error.epsilon_Z, rgs.N, M, p_dc=error.BSM_error)
...
if __name__ == "__main__":
    ...
    for i in range(2):
        ...
        rgs = RGS_ancilla(BRANCH_PARAM[i], 1, miu=miu, detector=tgs.Detector(eta_d=1, p_dc=0.1))
        time = tgs.Time(1, 500)
        error = RGS_Error(rgs, time, t_spin_coherence=1, epsilon_sp_measure_set=2.5e-2)
        # print(rgs.logical_qubit.miu_total())
        print(RGS_fidelity(error.error_leafs, error.epsilon_X, error.epsilon_Z, rgs.N, 1, p_dc=error.BSM_error))
