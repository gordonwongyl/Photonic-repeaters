...
def delay_line_tgs_ancilla(branch_param: np.ndarray, time: tgs.Time) -> float:
    t_P = time.P_a
    t_E = time.E_a
    t_E_long = time.E_a_long_photon
    t_CZ = time.CZ_a
    beta = time._beta

    tree = tgs.Tree_ancilla(branch_param=branch_param, miu=tgs.Miu(L=0., L_att=0., L_delay=0., L_feedback=0., m=0, miu_set=0.))
    d = tree.depth
    n_P = tree.num_qubits(d, d)/tree.b[0]
    n_E = tree.num_qubits(2,d-1)/tree.b[0] 

    n_CZ = tree.num_qubits(2, d-1)/tree.b[0] + 3 # Is this correct?

    # return (3*t_CZ + tree.b[1]*(tree.b[2]*t_P + t_E + t_CZ) + 2*t_E_long)*V_DELAY
    return float((n_P*t_P + n_E*t_E + 2*t_E_long + n_CZ*t_CZ)*V_DELAY)
...
def feedback_line_tgs_feedback(branch_param: np.ndarray, time: tgs.Time) -> float:
    t_E = time.E_f
    t_P = time.P_f
    tree = tgs.Tree_feedback(branch_param=branch_param, miu=tgs.Miu(L=0., L_att=0., L_delay=0., L_feedback=0., m=0, miu_set=0.))
    d = tree.depth
...
def delay_line_rgs_ancilla(branch_param: np.ndarray, n: int, time: tgs.Time) -> float:
    ...
    logical_qubit = tgs.Tree_ancilla(branch_param=branch_param, miu=tgs.Miu(L=0., L_att=0., L_delay=0., L_feedback=0., m=0, miu_set=0.))
...
def feedback_line_rgs_feedback(branch_param: np.ndarray, n: int, time: tgs.Time) -> float:
    ...
    t_E = time.E_f
    t_P = time.P_f
    logical_qubit = tgs.Tree_feedback(branch_param=branch_param, miu=tgs.Miu(L=0., L_att=0., L_delay=0., L_feedback=0., m=0, miu_set=0.))
    d = logical_qubit.depth
