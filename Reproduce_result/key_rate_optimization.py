...
class tgs_a_routine(Routine):
    def __init__(self, L: float, GAMMA: float, T_SPIN_COH: float) -> None:
        ...
        self.gene_space = [range(1, 11), range(1, 30), range(1, 30), {"low": 1., "high": 2000.}]
...
def optimization(input):
    ...
    for iter in range(3):
        rout = input["rout"](input["L"], input["GAMMA"], input["T_SPIN_COHERENCE"])
        ...
        filename = "optimized_result/" + rout.__class__.__name__ +f"_gamma_{input['GAMMA']}_spin_coh_{input['T_SPIN_COHERENCE']}_L_{input['L']}_iter_{iter+1}"
