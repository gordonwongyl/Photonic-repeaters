...
def delay_line_tgs_ancilla(branch_param: np.ndarray, time: tgs.Time) -> float:
    ...
    n_E = tree.num_qubits(2,d-1)/tree.b[0] + beta
    n_CZ = tree.num_qubits(2, d-1)/tree.b[0] # Is this correct?
    ...
    # n_CZ = b[1]
    # b = branch_param
    # return np.float128(b[1]*b[2]*t_P + (beta + b[1])*t_E + b[1]*t_CZ)*V_DELAY
...
if __name__ == "__main__":
    ...
    for i in range(4):
        ...
        print(time.P_a, time.E_a, time.H, time.E_f, time.CZ_a, time.CZ_f, time.M)
    ...
    # BRANCH_PARAM = [np.array([2,3]), np.array([3,1,1,1]), np.array([3,1,1,1,1])]
    # for i in range(1):
    #     name = 'tgs_f'
    #     time = tgs.Time(GAMMA[0])
    #     print(name + ": feedback_line = {:.2f}".format(feedback_line_tgs_feedback(BRANCH_PARAM[i], time)))
    #     print(name + ": delay_line = {:.2f}".format(delay_line_tgs_feedback(BRANCH_PARAM[i], time)))
    #     print()

    # tree = tgs.Tree_ancilla(np.array([2,3]), miu=tgs.Miu(L=0., L_att=0., L_delay=0., m=0, miu_set=0.))
    # print(tree.num_qubits(2,2))
